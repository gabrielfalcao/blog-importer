...
class GoogleLogin(object):
    ...
    @property
    def scopes(self):
        return self.app.config.get('google_oauth_scopes', '')

    @property
    def client_id(self):
        return self.app.config['google_oauth_client_id']

    @property
    def client_secret(self):
        return self.app.config['google_oauth_client_secret']

    @property
    def redirect_uri(self):
        return self.app.config.get('google_oauth_redirect_uri')

    @property
    def redirect_scheme(self):
        return self.app.config.get('google_oauth_redirect_scheme', 'http')
    ...
    def oauth2callback(self, view_func):
        ...
        def decorated(*args, **kwargs):
            ...
            if code:
                ...
                user = self.get_userinfo(token['access_token'])
                params.update(token=token, user=user)
